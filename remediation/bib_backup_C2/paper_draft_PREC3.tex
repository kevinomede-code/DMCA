...
Lag-Llama~\cite{rasul2024lagllama} adapts the LLaMA decoder
...
\end{document}
9.1\,\% coverage,
$-$20\,s lead time on Bearing3\_2) is the difference between an
actionable safety alarm and a missed failure.
The DMCA-Drift classifier achieves 92\,\% accuracy across seven
drift types with FAR\,=\,0.000, enabling type-aware re-alignment
that avoids unnecessary model swaps for transient events.
End-to-end re-alignment on ETT-h1 completes in 20 timesteps with
a 78\,\% MAE reduction, confirming the closed-loop dynamics.
The conversational XAI copilot and COPILOTCONFIRM gate provide
non-technical operators with structured, auditable involvement in
swap decisions for safety-critical contexts.

Future work will validate the $10\times$ T4-to-Jetson scaling
factor on physical hardware, extend DMCA to multivariate and
multi-asset scenarios with heterogeneous SLA profiles, integrate
the BaSyx SDK for Type~2 reactive AAS streaming, and evaluate
ONNX quantisation of admitted models to further compress the
admissible latency envelope.
...
\bibitem{IDTA_AAS}
Plattform Industrie~4.0 / IDTA, ``Details of the Asset Administration
Shell, Part~1: The Exchange of Information Between Partners in the
Value Chain of Industrie~4.0,'' v3.0, IDTA, 2023.
...
\textit{arXiv:2402.02592}, 2024.

\bibitem{Rasul2023LagLlama}
K.~Rasul, A.~Ashok, A.R.~Williams \textit{et al.},
``Lag-Llama: Towards foundation models for probabilistic time series
forecasting,'' \textit{arXiv:2310.08278}, 2023.

\bibitem{Hwang1981TOPSIS}
C.-L.~Hwang and K.~Yoon, \textit{Multiple Attribute Decision Making:
Methods and Applications}. Springer, Berlin, 1981.

\bibitem{Shi2016EdgeComputing}
W.~Shi, J.~Cao, Q.~Zhang, Y.~Li, and L.~Xu, ``Edge computing:
Vision and challenges,'' \textit{IEEE Internet Things J.}, vol.~3,
...
for bearings accelerated degradation tests,'' in \textit{IEEE Int.
Conf. Prognostics Health Management (PHM)}, 2012.

\bibitem{He2025XAI}
G.~He, ``Explainability for human-AI trust in industrial AI systems,''
